...
void drawRect(double x1, double y1, double width, double height, unsigned int color)
{
    glBegin(GL_LINES);
        glColor3ub((color >> 16 & 0xFF), (color >> 8 & 0xFF), (color & 0xFF));
        glVertex2d(x1, y1);
        glVertex2d(x1+width-0.5, y1);
        glVertex2d(x1, y1);
        glVertex2d(x1, y1+height-0.5);
        glVertex2d(x1+width-0.5, y1);
        glVertex2d(x1+width-0.5, y1+height-0.5);
        glVertex2d(x1, y1+height-0.5);
        glVertex2d(x1+width-0.5, y1+height-0.5);
    glEnd();
}
...
//    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, img_width, img_height, 0, GL_RGB, GL_UNSIGNED_BYTE, img);
    ...
    double lastMousePosX = -100;
    double lastMousePosY = -100;
...
//        glOrtho(0, width, height, 0, 0 , 1);
        ...
        {
            mousePositionCallback(window, mousePosX, mousePosY);
